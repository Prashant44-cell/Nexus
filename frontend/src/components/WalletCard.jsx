...
export default function WalletCard({ credential, isRevoked }) {
  return (
    <div className={`glass-panel neon-glow-hover ${isRevoked ? '' : 'glow-border'}`} style={{
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.25rem' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
          <div style={{
            padding: '0.6rem',
            background: isRevoked ? 'rgba(239,68,68,0.15)' : 'rgba(26, 153, 117, 0.15)',
            borderRadius: '12px',
            border: `1px solid ${isRevoked ? 'rgba(239,68,68,0.3)' : 'rgba(26, 153, 117, 0.3)'}`
          }}>
            {isRevoked ? <UserX size={24} color="#ef4444" /> : <UserCheck size={24} color="#1a9975" />}
          </div>
          <div>
            <h3 style={{ fontSize: '1.15rem', color: 'var(--text-main)' }}>Digital Identity Wallet</h3>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.85rem' }}>
        <div style={{ background: 'var(--bg-page)', border: '1px solid var(--border)', padding: '0.85rem', borderRadius: '10px' }}>
          <span style={{ color: 'var(--text-muted)', fontSize: '0.75rem', display: 'block' }}>FULL NAME</span>
          <strong style={{ fontSize: '1.05rem', color: 'var(--text-main)' }}>{credential?.full_name || '—'}</strong>
        </div>

        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.75rem' }}>
          <div style={{ background: 'var(--bg-page)', border: '1px solid var(--border)', padding: '0.75rem', borderRadius: '10px' }}>
            <span style={{ color: 'var(--text-muted)', fontSize: '0.75rem', display: 'flex', alignItems: 'center', gap: '0.35rem' }}>
              <Building2 size={13} /> INSTITUTION
            </span>
            <span style={{ fontSize: '0.85rem', fontWeight: '600', color: 'var(--text-main)' }}>{credential?.institution || '—'}</span>
          </div>
          ...
          <div style={{ background: 'var(--bg-page)', border: '1px solid var(--border)', padding: '0.75rem', borderRadius: '10px' }}>
            ...
            <span style={{ fontSize: '0.85rem', fontWeight: '600', textTransform: 'capitalize', color: 'var(--text-main)' }}>
        ...
        <div style={{ background: 'var(--bg-page)', border: '1px solid var(--border)', padding: '0.75rem', borderRadius: '10px' }}>
          ...
          <code className="mono-font" style={{ fontSize: '0.75rem', color: '#1a9975', wordBreak: 'break-all' }}>
